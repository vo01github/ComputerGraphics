...
void clippedWidowsInit()
{
	...
	ClippingSize.RightUpPoint.y = ScreenSize.y * scaleScreen * (scale);
}
...
// 判断一条直线 与矩形框的 4条直线相交的点的个数
// 然后算出 被截下来的直线
bool clippedLineFromRect(Line2D & checkLine, const Rect2D & checkRect)
{
	// 四条特殊的直线
	float xR	=	checkRect.RightUpPoint.x;
	float xL	=	checkRect.LeftDownPoint.x;
	float yUp	=	checkRect.RightUpPoint.y;
	float yDown	=	checkRect.LeftDownPoint.y;

	std::vector<float>  xVertical;
	xVertical.push_back(xR);
	xVertical.push_back(xL);

	std::vector<float>  yLevel;
	yLevel.push_back(yUp);
	yLevel.push_back(yDown);
...
std::vector<Point2D> twoLineIntersection(const Line2D & line1, const std::vector<float> & xVertical, const std::vector<float> & yLevel)
{
	...
	for (float Xcon : xVertical)
	{
		...
		// 发生相交的情况
		if (small < Xcon && Xcon < big) {
			Point2D crossPoint;
			crossPoint.x = Xcon;
			crossPoint.y = line1.startPos.y + k1*(Xcon - line1.startPos.x);
			result.push_back(crossPoint);
		} 	 	
	}


	for (float Ycon : yLevel)
	{
		...
		// 发生相交的情况
		if (small < Ycon && Ycon < big) {
			Point2D crossPoint;
			crossPoint.x = line1.startPos.x + (Ycon - line1.startPos.y)/k1;
			crossPoint.y = Ycon;
			result.push_back(crossPoint);
		} 	 	
	}
